...
void Product(vector<vector<int *> *> A, vector<vector<int *> *> B, vector<vector<int *> *> &C, int &GNo, vector<int> T,
             int N) {
    vector<vector<int> > S;
    C.clear();
    S.resize(N);
            ...
            if (ok_to_merge) {
                ...
                for (unsigned int m = 0; m < output.back()->Cand_id.size(); m++) {//petla do sprawdzania kandydata
                    ...
                    for (unsigned int n = 0;
                         n < output.back()->Cand_id.size(); n++) {//petla do tworzenia sprawdzianow dla kandydata
                        if (n != m)
                    ...
                    if (cand_validate ==
                        false) {//jezeli nie spelnia to nie sprawdzaj dalszych sprawdzianow i usun kandydata
                        for (unsigned int z = 0; z < output.back()->groups.size(); z++) {//wszystkie grupy
                            for (unsigned int a = 0;
                                 a < output.back()->groups.at(z)->size(); a++) {//usun wszystkie elementy z grupy
    ...
    for (unsigned int i = 0; i < Ck.size(); i++)
        for (unsigned int j = i + 1; j < Ck.size(); j++) {
            ...
            if (ok_to_merge) {
                ...
                output.back()->Cand_id = Ck.at(i)->Cand_id;//dodaj id atrybutów z pierwszego dziecka
                output.back()->Cand_id.push_back(Ck.at(j)->Cand_id.at(k - 1));// i ostatni id z drugiego dziecka
                StrippedProduct(Ck.at(i)->groups, Ck.at(j)->groups, Ck.at(j)->GroupsNo, output.back()->groups,
                                output.back()->GroupsNo, T, N);
                bool cand_validate = false;
                vector<int> cand_validator;
                for (unsigned int m = 0; m < output.back()->Cand_id.size(); m++) {//petla do sprawdzania kandydata
                    cand_validator.clear();
...
void StrippedProduct(vector<vector<int *> *> A, vector<vector<int *> *> B, int BGroupNo, vector<vector<int *> *> &C,
                     int &GNo, vector<int> T, int N) {
    vector<vector<int> > S;
    C.clear();
    S.resize(N);
...
void DecisionProduct(vector<int> delta, vector<vector<int *> *> A, vector<vector<int *> *> B, Candidate *&C,
                     vector<int> T, int N) {
    vector<vector<int> > S;
    C->groups.clear();
    C->GroupsNo = 0;
