...
bool Menu(int *d_column, int *d_type, int *a_type, ifstream *input_file) {
    ...
    if(!input_file->is_open()) {
        ...
        (*input_file).open(inputFileName);
        if(!input_file->is_open()) {
            cout<<"Nie prawidłowa nazwa pliku (plik nie istnieje) - sprawdź czy plik jest w tym samym folderze co plik wykonywalny programu."<<endl;
            return false;
        }
    }
    ...
    d_column << " , rodzaj decyzji - " << d_type << " , rodzaj algorytmu - " << a_type << endl;
    return true;
}
